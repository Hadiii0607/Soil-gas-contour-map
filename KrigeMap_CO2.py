...
df =  pd.read_excel('File_name.xlsx', sheet_name= 'CO2')
# 从DataFrame中删除五行
...
df_la = df['纬度'].values
df_lo = df['经度'].values
df_rn = np.log10(df['二氧化碳'].values)
#df_rn = df['二氧化碳'].values
...
Krin =  OrdinaryKriging(lof, lat, rnf, variogram_model= 'spherical', variogram_parameters= Parameter, nlags=6, exact_values = True, coordinates_type='geographic')
